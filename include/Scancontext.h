...
public:
    ...
    int scdPCNumRing;
    int scdPCNumSector;
    double scdPCMaxRadius;

    ParamServerSCD()
    {
        nh.param<int>("lio_sam/scdPCNumRing", scdPCNumRing, 20);
        nh.param<int>("lio_sam/scdPCNumSector", scdPCNumSector, 60);
        nh.param<double>("lio_sam/scdPCMaxRadius", scdPCMaxRadius, 80.0);
        usleep(100);
    }
};

class SCManager : public ParamServerSCD
{
public: 
    ...
    // for insert 
    void loadMakeSaveScancontextAndKeys( Eigen::MatrixXd & sc );
...
public:
    ...
    const int    PC_NUM_RING = scdPCNumRing;        // 20 in the original paper (IROS 18)
    const int    PC_NUM_SECTOR = scdPCNumSector;    // 60 in the original paper (IROS 18)
    const double PC_MAX_RADIUS = scdPCMaxRadius;    // 80 meter max in the original paper (IROS 18)
